...
// Large whole numbers use integer form up to the 1e21 fixed-notation window
// (matching go-jsonnet), and normalized scientific notation beyond it.
std.assertEqual("x=" + 1e15, "x=1000000000000000") &&
std.assertEqual("x=" + 1e20, "x=100000000000000000000") &&
std.assertEqual("x=" + 1e21, "x=1e+21") &&
// Past the window the compact form is required, not merely preferred: since the
// numeric rework a whole value is an exact Dec128 whose exponent range is far wider
// than binary64's, so expanding every whole value would spell 1e6000 as six thousand
// characters. See RenderUtils.renderDec128.
std.assertEqual(std.manifestJson(1e100), "1e+100") &&
std.assertEqual(std.manifestToml({a: 1e100}), "a = 1e+100") &&
