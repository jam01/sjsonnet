...
object NumberMathTests extends TestSuite {
  ...
  def tests: Tests = Tests {
    ...
    test("float64 literal admission") {
      ...
      test("floatAsBigDecimal does not gate arithmetic promotion") {
        // The flag is a parsing/representation switch only; Float64 ⊕ Float64 promotes either
        // way. NumberMath.promoteFloat64Arithmetic is the single place that decides, so a future
        // speed escape hatch is one edit rather than five.
        assert(NumberMath.promoteFloat64Arithmetic)
      }
    }
